...
class Emojis:
    """Just emojis"""
    check = "<:saint_approve:1159932048560111726>" #<:check:1126571720816468129>
    wrong = "<:saint_decline:1159932108844834817>" #<:cross:1126572030750371985>
    warning = "<:saint_warning:1159932006470258688>" #{Emojis.warning}
...
async def embed_replacement(user, params):
    ...
    if "{user.joined_at}" in params:
        params = params.replace(
            "{user.joined_at}", discord.classes.format_dt(user.joined_at, style="R")
        )
    if "{user.created_at}" in params:
        params = params.replace(
            "{user.created_at}", discord.classes.format_dt(user.created_at, style="R")
        )
    ...
    if "{guild.created_at}" in params:
        params = params.replace(
            "{guild.created_at}",
            discord.classes.format_dt(user.guild.created_at, style="R"),
        )
