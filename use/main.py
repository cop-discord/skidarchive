...
async def status(): 
   list = [f"{len(bot.guilds)} servers", ",help", "discord.gg/dramatic", f"{psutil.cpu_percent()}% cpu usage"]
    ...
    async with bot.db.cursor() as cursor:
        ...
        for result in results:
            ...
            try:
                ...
                msg = await channel.send(embed=eembed) ##eembed
...
async def male():
    ...
    async with bot.db.cursor() as cursor:
        ...
        for result in results:
            ...
            try:
                channel = bot.get_channel(channel_id)
                msg = await channel.send(embed=eembed)
                await msg.add_reaction("<a:loading:1129810812995911730>")
                await asyncio.sleep(1.50)
                await msg.clear_reactions()
                await asyncio.sleep(1.90)
...
class Client(commands.AutoShardedBot): #commands.AutoShardedBot 
  def __init__(self):
    super().__init__(
        ...
        owner_ids=[1094511683554267197, 749674886963920956, 461914901624127489],
