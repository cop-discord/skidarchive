import discord, aiosqlite ; from discord.ext import commands ; from cogs.events import Emojis ; from utils.classes import Colors
...
class reactionrole(commands.Cog):
    ...
    async def reactionrole(self, ctx):
        if ctx.subcommand_passed is not None:
            return
        embed = discord.Embed(color=Colors.default, title="reaction roles", description="have our bot give a user a role when they react to a message")
    ...
    async def list(self, ctx):
        ...
        embed = discord.Embed(title='reaction role list', color=0x495063)
        for row in rows:
