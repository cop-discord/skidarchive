...
class boosters(commands.Cog):
    ...
    @commands.hybrid_group(aliases = ['bm'])
    async def boost(self, ctx):
        if ctx.invoked_subcommand is None:
            return await ctx.reply(f"{ctx.author.mention}: maybe try ,h boost?")

    @boost.command(description='boost')
            ...
            async with self.bot.db.cursor() as cursor:
                ...
                if channelData:
                    ...
                    if channelData == channel.id:
                        embed = discord.Embed(description=f"{Emojis.wrong} {ctx.author.mention}: The **boost channel** is already set as {channel.mention}", color = 0x2f3136)
                        return await ctx.reply(embed=embed)
                    await cursor.execute("UPDATE boostconfig SET channel = ? WHERE guild = ?", (channel.id, ctx.guild.id,))
                    embed = discord.Embed(color = Colors.default, description = f"{Emojis.check} {ctx.author.mention}: Successfully binded the **boost channel** to {channel.mention}")
    ...
    @boost.command(aliases = ['msg'], description='boost')
    @commands.has_permissions(manage_guild = True)
        ...
        try:
            async with self.bot.db.cursor() as cursor:
                ...
                if starData:
                    starData = starData[0]
                    if starData == message:
                        embed = discord.Embed(description=f"{Emojis.wrong} {ctx.author.mention}: That's already set as the **boost message**", color = Colors.default)
                        return await ctx.reply(embed=embed)
                    await cursor.execute("UPDATE boostconfig SET message = ? WHERE guild = ?", (message, ctx.guild.id,))
                    embed = discord.Embed(color = Colors.default, description = f"""{Emojis.check} {ctx.author.mention}: Successully updated the **boost message** to the following:
```{message}```""")
                    await ctx.reply(embed=embed)
                else:
    ...
    async def delete(self, ctx):
            try:
                async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(color = Colors.default, description = f"""Successully **delete** the boost message module""")
                    await ctx.reply(embed=embed)
                await self.bot.db.commit()
            except Exception as e:
                print(e)
