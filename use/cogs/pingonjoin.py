...
class pingonjoin(commands.Cog):
    ...
    async def on_ready(self):
        async with self.bot.db.cursor() as cursor:
            await cursor.execute("CREATE TABLE IF NOT EXISTS pingonjoin (channel INTEGER, guild_id INTEGER);")
    ...
    async def on_member_join(self, member: discord.Member):
        ...
        async with self.bot.db.cursor() as cursor:
            await cursor.execute("SELECT * FROM pingonjoin WHERE guild_id = ?", (member.guild.id,))
    ...
    async def pingonjoin(self, ctx):
        if ctx.invoked_subcommand is None:
            e = discord.Embed(title="Command: pingonjoin", description="ping new members when they join your server",
                              color=Colors.default, timestamp=ctx.message.created_at)
    ...
    async def add(self, ctx: commands.Context, *, channel: discord.TextChannel):
        ...
        try:
            async with self.bot.db.cursor() as cursor:
                await cursor.execute("INSERT INTO pingonjoin (channel, guild_id) VALUES (?, ?)", (channel.id, ctx.guild.id))
            embed = discord.Embed(description=f"> {Emojis.check} {ctx.author.mention}: successfully added ping new members on {channel.mention}",
    ...
    async def clear(self, ctx):
        ...
        try:
            async with self.bot.db.cursor() as cursor:
                await cursor.execute("DELETE FROM pingonjoin WHERE guild_id = ?", (ctx.guild.id,))
            embed = discord.Embed(description=f"> {Emojis.check} {ctx.author.mention}: successfully cleared ping on join",
                                  color=Colors.default)
            await sendmsgg(self, ctx, None, embed, None, None, None, None)
            await self.bot.db.commit()
        except Exception as e:
    ...
    async def show(self, ctx: commands.Context):
        ...
        try:
            async with self.bot.db.cursor() as cursor:
                await cursor.execute("SELECT * FROM pingonjoin WHERE guild_id = ?", (ctx.guild.id,))
