...
from backend.prem import premium
from backend.psee import PaginatorView
from backend.https import HTTP
...
class fun(commands.Cog):
    ...
    @commands.hybrid_command(description="fun", usage="[word]")
    ...
    async def randomnumber(self, ctx, *, digits:int=1):
        ...
        await ctx.reply(number, mention_author=True)
    
    @commands.hybrid_command(help="shows your iq", description="fun", usage="<member>")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @blacklist()
    async def iq(self, ctx, user: discord.Member=None):

        if user==None:
            embed=discord.Embed(color=0x2B2D31, title="iq test", description= f"{ctx.author.mention} has `{random.randrange(201)}` iq :brain:")
            await ctx.reply(embed=embed, mention_author=False)
        else:
            embed=discord.Embed(color=0x2B2D31, title="iq test", description= f"{user.mention} has `{random.randrange(201)}` iq :brain:")
            await ctx.reply(embed=embed, mention_author=False)
            
    @commands.hybrid_command(help="send a random picture of a cat", description="fun")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @blacklist()
    async def cat(self, ctx):
        async with aiohttp.ClientSession() as cs:
            async with cs.get("https://www.reddit.com/r/cat.json") as r:
                cat = await r.json()
                embed = discord.Embed(color=0x2B2D31, title="kitty")
                embed.set_image(url=cat["data"]["children"][random.randint(0, 25)]["data"]["url"])
                await ctx.reply(embed=embed, mention_author=False)
                
    @commands.hybrid_command(help="send a random picture of a dog", description="fun")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @blacklist()
    async def dog(self, ctx):
        async with aiohttp.ClientSession() as cs:
            async with cs.get("https://www.reddit.com/r/dog.json") as r:
                dog = await r.json()
                embed = discord.Embed(color=0x2B2D31, title="doggo")
                embed.set_image(url=dog["data"]["children"][random.randint(0, 25)]["data"]["url"])
                await ctx.reply(embed=embed, mention_author=False)
    
    @commands.hybrid_command(help="shows how gay you are", description="fun", usage="<member>")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @blacklist()
    async def howgay(self, ctx, user: discord.Member=None):

        if user==None:
            embed=discord.Embed(color=0x2B2D31, title="gay r8", description= f"{ctx.author.mention} is `{random.randrange(201)}%` gay")
            await ctx.reply(embed=embed, mention_author=False)
        else:
            embed=discord.Embed(color=0x2B2D31, title="gay r8", description= f"{user.mention} is `{random.randrange(201)}%` gay")
            await ctx.reply(embed=embed, mention_author=False)
    ...
    @commands.hybrid_command(help="shows how many bitches you have", description="fun", usage="<member>")
    @commands.cooldown(1, 3, commands.BucketType.user)
    @blacklist()
    async def bitches(self, ctx, user: discord.Member=None):

        if user==None:
            embed=discord.Embed(color=0x2B2D31, description= f"{ctx.author.mention} has `{random.randrange(51)}` bitches")
            await ctx.reply(embed=embed, mention_author=False)
        else:
            embed=discord.Embed(color=0x2B2D31, description= f"{user.mention} has `{random.randrange(51)}` bitches")
            await ctx.reply(embed=embed, mention_author=False)
    
    @commands.hybrid_command(help="make the bot say anything", description="fun", usage="[word]", aliases=["poll"])
    @commands.cooldown(1, 3, commands.BucketType.user)
    @blacklist()
    async def say(self, ctx, *, text):
        ...
        embed=discord.Embed(color=0x2B2D31, title=f"{ctx.author} said...",description=f"{text}")
