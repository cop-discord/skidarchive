...
class lastfm(commands.Cog):
            ...
            async with message.channel.typing():
              ...
              if res is not None:   
                 ...
                 if user != "error":   
                    ...
                    embed.set_footer(text = f"Track Playcount: {await lastfmhandler.get_track_playcount(user, a['recenttracks']['track'][0])} ă»Album: {album}", icon_url = (a['recenttracks']['track'][0])['image'][3]['#text'])
                    await message.reply(embed=embed, mention_author=True) 
              elif check is None: 
                embed = discord.Embed(description=f"<:lf:1073306604201574432> **{member}** doesn't have a **Last.fm account** linked. Use `,lf set <username>` to link your **account**.", color=Colors.red)
                await message.reply(embed=embed, mention_author=True)  
    ...
    @commands.hybrid_group(aliases = ['lf'])
    @blacklist()
...
through the bot!""",color = 0x2B2D31)
...
>   the other commands are displayed in the help command""", inline = False)
    ...
    async def customcommand(self, ctx, *, cmd=None):
        ...
        async with self.bot.db.cursor() as cursor:
                    ...
                    embed = discord.Embed(description=f'<:lf:1073306604201574432> {ctx.message.author.mention}: Your **Last.fm** custom command is {cmd}', color=Colors.red)
                    #await sendmsg(self, ctx, None, embed, None, None, None)   
                    embed_msg = await ctx.send(embed=embed)
                    await embed_msg.add_reaction("<:thumbsup:1129811199626854534>")
                    #await embed_msg.add_reaction("đź‘Ž")
                    await self.bot.db.commit()  
            ...
            if check is not None:   
                ...
                if user != "error": 
                 async with aiohttp.ClientSession() as session:
                    params= {"api_key" : "9835f9884e1bb2b122252ed77a39c5be",
                    "user" : user,
                    "period" : "overall",
                    "limit" : 10,
...
`10` **[{topartist10}]({topartist10url})** {topartist10plays} plays""",color = 0x2B2D31)
...
`10` **[{toptrack10}]({toptrack10url})** {toptrack10plays} plays""",color = 0x2B2D31)
...
`10` **[{topalbum10}]({topalbum10url})** {topalbum10plays} plays""",color = 0x2B2D31)
                        embed.set_thumbnail(url = ctx.message.author.avatar)
      ...
      async with ctx.typing():
       async with self.bot.db.cursor() as cursor:
        ...
        for check in results:
         ...
         if username != "error": 
          async with aiohttp.ClientSession() as session:
           params= {"api_key" : "9835f9884e1bb2b122252ed77a39c5be", "user": username, "artist" : artist, "method": "artist.getInfo", "autocorrect": "true", "format":"json"}
           async with session.get(url="http://ws.audioscrobbler.com/2.0", params=params) as response:
            resp = await response.read()
            jsonData = json.loads(resp)
            try:
