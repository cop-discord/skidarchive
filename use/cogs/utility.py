...
class Utilityy(Cog):
    ...
    async def seen(self, ctx, *, member: Member=None):
      ...
      async with self.bot.db.cursor() as cursor: 
        ...
        check = await cursor.fetchone()
        if check is None: return await sendmsg(self, ctx, None, Embed(color=Colors.yellow, description=f"<:warn:1047734726586286080> {ctx.author.mention}: I didn't see **{member}**"), None, None, None) 
        ts = check[2]
