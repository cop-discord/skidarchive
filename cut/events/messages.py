...
class Messages(commands.Cog):
        ...
        if "MessageType.premium_guild" in str(message.type):
            if message.guild.id == 1060502285383376916:
                ...
                return await message.channel.send(
                    f"{member.mention} ty for boosting, enjoy your perks! <:joobiheart:1083091785015885836>"
                )
    ...
    async def afk_listener(self, message: discord.Message):
        ...
        if message.mentions:
            if len(message.mentions) == 1:
                ...
                if check:
                    em = discord.Embed(
                        color=self.bot.color,
                        description=f"**{mem}** is AFK since <t:{int(check['time'])}:R> - **{check['reason']}**",
                    )
        ...
        if che:
            embed = discord.Embed(
                color=self.bot.color,
                description=f"You're back **{message.author}**, your AFK has been removed",
            )
