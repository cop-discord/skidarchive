...
class Users(commands.Cog):
    ...
    async def auto_or_jail(self, member: discord.Member):
        ...
        if check:
            ...
            if chec:
                try:
                    await member.add_roles(member.guild.get_role(int(chec["role_id"])))
                except:
                    pass
        elif not check:
            role = None
            ...
            if len(results) == 0:
                return
            roles = [
                member.guild.get_role(int(result["role_id"]))
                for result in results
                if member.guild.get_role(int(result["role_id"])) is not None
                and member.guild.get_role(int(result["role_id"])).is_assignable()
            ]
            if role:
                roles.append(role)
    ...
    async def on_user_update(self, before, after):
        ...
        await self.bot.db.execute(
            "INSERT INTO oldusernames VALUES ($1,$2,$3)",
            before.name,
    ...
    async def booster_unboosted(self, before: discord.Member, after: discord.Member):
        if before.guild.id == 1060502285383376916:
            if (
                ...
                check = await self.bot.db.fetchrow(
                    "SELECT * FROM donor WHERE user_id = {}".format(before.id)
                )
                if check is not None:
                    return await self.bot.db.execute(
                        "DELETE FROM donor WHERE user_id = {}".format(before.id)
                    )
    ...
    async def on_member_update(self, before: discord.Member, after: discord.Member):
        if str(before.nick) != str(after.nick):
            ...
            if check:
                return await before.edit(nick=check["nickname"])
