...
class Paginator(View):
    ...
    async def on_error(self, error, item, interaction):
        ...
        print(
            f"Error in {self.__class__.__name__}: {error} (item: {item}, interaction: {interaction})"
        )
...
class PaginatorButton(Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if self.custom_id == "previous":
            ...
            _message = await interaction.neutral(
                "What **page** would you like to skip to?", emoji="🔢"
            )
            try:
                ...
                await self.view.message.edit(
                    embed=self.view.embeds[self.view.current_page], view=self.view
                )
                with contextlib.suppress(discord.HTTPException):
                    await _message.delete()
