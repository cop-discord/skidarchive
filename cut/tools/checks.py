...
class Pot:
    def check_pot():
        async def predicate(ctx: commands.Context):
            check = await ctx.bot.db.fetchrow(
                "SELECT * FROM pot WHERE guild_id = $1", ctx.guild.id
            )
            if not check:
                await ctx.bot.ext.send_error(
                    ctx,
                    f"This server doesn't have a **pot**. Use `{ctx.clean_prefix}pot toggle` to get one",
                )
            return check is not None
    ...
    def pot_owner():
        async def predicate(ctx: commands.Context):
            ...
            if check["holder"] != ctx.author.id:
                await ctx.send_warning(
                    f"You don't have the **pot**. Steal it from <@{check['holder']}>"
                )
            return check["holder"] == ctx.author.id
