class No_Response(Exception):
    ...
    def __init__(
        self,
        message="Tiktok returned no response, this might just be a check on their backend or some error. Try again.",
    ):
...
class msToken(Exception):
    ...
    def __init__(
        self,
        func,
        message="Could not find any msToken passed in the TikTokApi class kwargs",
    ):
        message += f" ({func})"
        self.message = message
        super().__init__(self.message)
...
class web_user_agent(Exception):
    ...
    def __init__(
        self,
        func,
        message="Could not find web_user_agent passed in the TikTokApi class kwargs, which is required when using ",
    ):
        message += func
        self.message = message
        super().__init__(self.message)
