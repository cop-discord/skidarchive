...
class Feed:
    def __init__(self, api):
        self.api = api
        self.trending_types = {1: "music", 0: "hashtag"}

    async def for_you(
        self, count="6", max_cursor="0", min_cursor="0", region="US", raw_data=False
    ) -> list:
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(
                    f"https://api2-19-h2.musical.ly/aweme/v1/feed/?type=0&max_cursor={max_cursor}&min_cursor={min_cursor}&count={count}&{req_from}&volume=0.2&pull_type={pull_type}&ts={ts}&_rticket={_rticket}&address_book_access=1&gps_access=2&os_api=25&device_type=SM-G973N&dpi=320&uoo=0&region={region}&carrier_region={region}&app_name=musical_ly"
                ) as feed_request:
            ...
            for vid in data:
                if raw_data:
                    ...
                    formatted_video_data = await self.api.video.video_data_formatter(
                        vid
                    )
    ...
    def format_categories(self, cat_data) -> list:
        ...
        for cat in cat_data:
            ...
            if t == 1:
                cat["music_info"]["extra"] = json.loads(
                    cat["music_info"]["extra"]
                )  # this returns string for some reason
                ...
                if len(beats) != 0:
                    category["music"][
                        "beats"
                    ] = beats  # no idea what the fuck this is, probably frames or something
            elif t == 0:
