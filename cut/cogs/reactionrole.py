...
class ReactionRoles(commands.Cog):
        ...
        *,
        ...
        if check:
            return await ctx.send_warning(
                "A reactionrole with the same arguments is **already** added"
            )
    ...
    async def rr_remove(
        ...
        if not check:
            return await ctx.send_warning(
                "**Cannot** find a reactionrole with the given arguments"
            )
        await self.bot.db.execute(
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(
                        ...
                        title=f"Reaction roles in {ctx.author.guild.name}, ({len(results)})",
