...
class greet(commands.Cog):
        ...
        if check:
            ...
            try:
                x = await EmbedBuilder.to_object(
                    EmbedBuilder.embed_replacement(member, check["message"])
                )
    ...
    async def leave(self, ctx):
        ...
        embed = discord.Embed(
            ...
            description=self.leave.short_doc or "No description provided",
        )
        embed.description += (
            f"\n>>> ```bf\nSyntax: {ctx.prefix}{self.leave.qualified_name} {self.leave.usage or ''}\n"
            + (
                f"Example: {ctx.prefix}{self.leave.qualified_name} {self.leave.help}"
                if self.leave.help
                else ""
            )
            + "```"
        ...
        embed.set_author(
            name=self.leave.cog_name or "No category",
            icon_url=self.bot.user.display_avatar,
            url=f"https://tear.lol/",
        )
        if isinstance(self.leave, commands.Group):
            embed.add_field(
                name="Subcommands",
                value="\n".join(
                    [
    ...
    @leave.command(
        name="variables",
        help="config",
        description="return the variables for the leave message",
    )
    async def leave_variables(self, ctx: commands.Context):
        await ctx.invoke(self.bot.get_command("embed variables"))

    @leave.command(
        name="config", help="config", description="returns stats of the leave message"
    )
    async def leave_config(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Welcome is **not** configured")
        channel = (
            f'#{ctx.guild.get_channel(check["channel_id"]).name}'
            if ctx.guild.get_channel(check["channel_id"])
            else "none"
        )
        e = check["mes"] or "none"
        embed = discord.Embed(
    ...
    async def leave_channel(
        ...
        if check:
            ...
            await self.bot.db.execute(
                "INSERT INTO leave VALUES ($1,$2,$3)", ctx.guild.id, channel.id, None
            )
    ...
    async def leave_message(self, ctx: commands.Context, *, code: str):
        ...
        if check:
            await self.bot.db.execute(
                "UPDATE leave SET message = $1 WHERE guild_id = $2", code, ctx.guild.id
            )
        else:
            await self.bot.db.execute(
    ...
    async def leave_delete(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Leave is **not** configured")
        else:
            await self.bot.db.execute(
                "DELETE FROM leave WHERE guild_id = $1", ctx.guild.id
            )
    ...
    async def leave_test(self, ctx):
        ...
        if check:
            ...
            return await ctx.send_success(
                "Leave message **sent** to {}".format(channel.mention)
            )
    ...
    async def welcome(self, ctx):
        ...
        embed = discord.Embed(
            color=self.bot.color,
            description=self.welcome.short_doc or "No description provided",
        )
        embed.description += (
            f"\n>>> ```bf\nSyntax: {ctx.prefix}{self.welcome.qualified_name} {self.welcome.usage or ''}\n"
            + (
                f"Example: {ctx.prefix}{self.welcome.qualified_name} {self.leave.help}"
                if self.welcome.help
                else ""
            )
            + "```"
        )
        embed.set_author(
            name=self.welcome.cog_name or "No category",
            icon_url=self.bot.user.display_avatar,
            url=f"https://tear.lol/",
        )
        if isinstance(self.leave, commands.Group):
    ...
    @welcome.command(
        name="variables",
        help="config",
        description="return the variables for the welcome message",
    )
    async def welcome_variables(self, ctx: commands.Context):
        await ctx.invoke(self.bot.get_command("embed variables"))

    @welcome.command(
        name="config", help="config", description="returns stats of the welcome message"
    )
    ...
    async def welcome_channel(
        ...
        if check:
            ...
            await self.bot.db.execute(
                "INSERT INTO welcome VALUES ($1,$2,$3)", ctx.guild.id, channel.id, None
            )
    ...
    async def welcome_message(self, ctx: commands.Context, *, code: str):
        ...
        return await ctx.send_success(
            f"Welcome message **configured** as\n```{code}```"
        )

    @welcome.command(
        name="delete", help="config", description="delete the welcome module"
    )
    @Perms.get_perms("manage_guild")
    ...
    async def welcome_test(self, ctx):
        ...
        if check:
            ...
            return await ctx.send_success(
                "Welcome message **sent** to {}".format(channel.mention)
            )
