    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        async def select_callback(inte: discord.Interaction):
            ...
            e = discord.Embed(
                ...
                description="Support will be with you shortly\nTo close the ticket please press 🔒",
            )
            e.set_footer(
                text="cut | tear.lol",
                icon_url=interaction.client.user.display_avatar.url,
...
class Tickets(commands.Cog):
    ...
    @ticket.command(
        description="manage the ticket topics", help="config", brief="administrator"
    )
    ...
    async def topics(self, ctx: commands.Context):
        ...
        async def button2_callback(interaction: discord.Interaction):
            ...
            e = discord.Embed(
                color=self.bot.color, description=f"🔍 Select a topic to delete"
            )
            options = []
            for result in results:
                options.append(
            ...
            async def select_callback(inter: discord.Interaction):
                ...
                await self.bot.ext.send_success(
                    inter, f"Removed **{select.values[0]}** topic", ephemeral=True
                )
    ...
    async def message(self, ctx: commands.Context, *, message: str = None):
        ...
        if message is not None:
            ...
            if check is None:
                return await ctx.send_warning("There is no custom ticket message")
            await self.bot.db.execute(
                "UPDATE tickets SET message = $1 WHERE guild_id = $2",
                None,
                ctx.guild.id,
    ...
    async def category(
        ...
        if channel is not None:
            ...
            if check is None:
                return await ctx.send_warning("tickets channel is not set")
            await self.bot.db.execute(
                "UPDATE tickets SET category = $1 WHERE guild_id = $2",
                None,
                ctx.guild.id,
    ...
    async def logs(self, ctx: commands.Context, *, channel: discord.TextChannel = None):
        ...
        if channel is not None:
            ...
            if check is None:
                return await ctx.send_warning("tickets logs are not set")
            await self.bot.db.execute(
    ...
    @ticket.command(
        description="sends the ticket panel", help="config", brief="administrator"
    )
    @utils.get_perms("administrator")
    async def send(self, ctx: commands.Context):
        ...
        if check is None:
            return await ctx.send_warning("No ticket panel created")
        ...
        await self.bot.db.execute(
            ...
            message.embeds[0].color.value or self.bot.color,
            ctx.guild.id,
        )
        await ctx.send_success(
            "Sent the **ticket** message to {}".format(channel.mention)
        )
    ...
    async def settings(self, ctx: commands.Context):
        ...
        embed.add_field(
            name="ticket channel",
            value=ctx.guild.get_channel(check["channel_id"]).mention
            if ctx.guild.get_channel(check["channel_id"]) is not None
            else "none",
        )
        embed.add_field(
            name="logs channel",
            value=ctx.guild.get_channel(check["logs"]).mention
            if ctx.guild.get_channel(check["logs"]) is not None
            else "none",
        )
        embed.add_field(
            name="category",
            value=ctx.guild.get_channel(check["category"]).mention
            if ctx.guild.get_channel(check["category"]) is not None
            else "none",
        )
        embed.add_field(
            name="message",
            value="```{}```".format(check["message"])
