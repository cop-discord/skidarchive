            ...
            if check:
                ...
                if any(link in self.children[0].value for link in links):
                    return await interaction.client.ext.send_warning(
                        interaction, "i cannot send links.".capitalize(), ephemeral=True
                    )
                embed = discord.Embed(
                    color=interaction.client.color,
                    description=f"{interaction.client.yes} {interaction.user.mention}: your confession has been sent in {channel.mention}",
                )
...
class Cog(commands.Cog):
    ...
    async def confess(self, ctx: discord.Interaction):
        ...
        return await self.bot.ext.send_error(
            ctx, "Confessions are **not** enabled in this server", ephemeral=True
        )

    @app_commands.command(name="poll", description="create a poll")
    async def poll(
        self, ctx: discord.Interaction, question: str, first: str, second: str
    ):
        embed = discord.Embed(
            color=ctx.client.color,
            title=question,
            description=f"1️⃣ - {first}\n\n2️⃣ - {second}",
        )
