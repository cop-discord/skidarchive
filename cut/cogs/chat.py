...
class Chat(commands.Cog):
    ...
    async def ar_add(self, ctx: commands.Context, *, arg: str):
        ...
        try:
            ...
            return await ctx.send_warning("No response has been found")
        check = await self.bot.db.fetchrow(
        ...
        if check:
            ...
            return await ctx.send_success(
                "Set response `{}` for the trigger `{}`".format(response, trigger)
            )
        else:
            await self.bot.db.execute(
    ...
    async def ar_remove(self, ctx: commands.Context, *, trigger: str):
        ...
        if not check:
            return await ctx.send_warning(
                f"Cannot find an autoresponse with the trigger `{trigger}`"
            )
        await self.bot.db.execute(
            ...
            if l == 10:
                embeds.append(
                    discord.Embed(
                        ...
                        title=f"{ctx.guild.name} Autoresponders, ({len(results)})",
                        description=mes,
                    )
                )
                l = 0
    ...
    async def autoreact_add(self, ctx: commands.Context, *, content: str):
        ...
        await ctx.send_success(
            f"Added autoreact for trigger **{con[0]}** - {''.join([e for e in emojis])}"
        )
    ...
    async def autoreact_remove(self, ctx: commands.Context, *, content: str):
        ...
        if not check:
            return await ctx.send_warning(
                f"No autoreact found for the trigger **{content}**"
            )
        await self.bot.db.execute(
        ...
        for result in check:
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(
                        color=self.bot.color,
                        title=f"{ctx.guild.name} Autoreactions, ({len(check)})",
