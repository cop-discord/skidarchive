...
lockemoji = "<:lock:1174788464617721927>"
unlockemoji = "<:unlock:1174788487019495554>"
ghostemoji = "<:ghost:1174787446114893834>"
unghostemoji = "<:unghost:1174787427836100648>"
channelemoji = "<:rename:1174787676331843677>"
plusemoji = "<:plus:1174787329563561994>"
minusemoji = "<:minus:1174787398719258624>"
claimemoji = "<:claim:1174787680505180203>"
manemoji = "<:info:1174787672896708709>"
hammeremoji = "<:kick:1174787669537067049>"
...
class vcModal(Modal, title="rename your voice channel"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.client.ext.send_error(
                interaction, f"an error occured - {e}", ephemeral=True
            )
...
class vmbuttons(discord.ui.View):
    ...
    async def lock(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if not check:
            return await interaction.client.ext.send_warning(
                interaction, "VoiceMaster is **not** configured", ephemeral=True
            )
        elif check:
    ...
    @discord.ui.button(
        ...
        emoji=hammeremoji,
        style=discord.ButtonStyle.gray,
        custom_id="persistent_view:manage",
    )
    async def manage(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if check is not None:
            ...
            em = discord.Embed(
                ...
                description="Moderate your voice channel using the options below",
            )
            options = [
...
class VoiceMaster(commands.Cog):
    ...
    async def get_channel_categories(
        ...
        if len(channel.category.channels) == 50:
            ...
            try:
                ...
                await member.send(
                    "I was **not able** to make a new voice channel.", view=view
                )
            except:
                pass
                    ...
                    if not channel.overwrites_for(member).connect == True:
                        try:
                            return await member.move_to(
                                channel=None,
                                reason="member not allowed to join this voice channel",
                            )
                        except:
                            pass
    ...
    @voice.command(
        description="rename the voice channel", help="config", usage="[name]"
    )
    @check_vc_owner()
    ...
    @voicemaster.command(
        description="get an updated version of the voice master interface",
        help="config",
    )
    ...
    @voicemaster.command(
        description="configure voicemaster module for your server",
        help="config",
        aliases=["set"],
    )
    @Perms.get_perms("administrator")
    async def setup(self, ctx: commands.Context):
        ...
        if check:
            return await ctx.send_warning("Voicemaster is **already** configured")
        elif not check:
            ...
            text = await ctx.guild.create_text_channel(
                "interface", overwrites=overwrite, category=category
            )
            ...
            return await ctx.send_success(
                "Voicemaster has **successfully** been configured"
            )
    ...
    async def remove(self, ctx):
        ...
        if not check:
            ...
            try:
                ...
                await ctx.send_success("Voicemaster has **successfully** been deleted.")
            except:
                await self.bot.db.execute(
                    "DELETE FROM voicemaster WHERE guild_id = $1", ctx.guild.id
                )
