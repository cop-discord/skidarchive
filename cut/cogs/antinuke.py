...
from tools.utils import Whitelist, EmbedBuilder, Paginator
...
def is_antinuke():
    async def predicate(ctx: commands.Context):
        ...
        if not check:
            await ctx.send_warning("Antinuke is **not** enabled")
...
class AntiNuke(commands.Cog):
    ...
    async def sendlogs(self, action: str, member: discord.Member, punishment: str):
        ...
        embed = discord.Embed(
            ...
            description="Was cut fast? If not, please let us know in https://discord.gg/your",
        )
        ...
        async def channel_delete(self, entry: discord.AuditLogEntry):
            if entry.action == discord.AuditLogAction.channel_delete:
                ...
                if (
                    ...
                    if punishment == "ban":
                        return await entry.user.ban(
                            reason="AntiNuke: Deleting channels"
                        )
                    elif punishment == "kick":
                        return await entry.user.kick(
                            reason="AntiNuke: Deleting channels"
                        )
                    else:
                        ...
                        if entry.user.bot:
                            for role in [
                                r for r in entry.user.roles if r.is_bot_managed()
                            ]:
                                await role.edit(
    ...
    @commands.group(
        ...
        name="antinuke",
        description="Protect your server against nukes",
        aliases=["an"],
    )
    async def antinuke(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            color=self.bot.color,
            description=self.antinuke.short_doc or "No description provided",
        )
        embed.description += (
            f"\n>>> ```bf\nSyntax: {ctx.prefix}{self.antinuke.qualified_name} {self.antinuke.usage or ''}\n"
            + (
                f"Example: {ctx.prefix}{self.antinuke.qualified_name} {self.antinuke.help}"
                if self.antinuke.help
                else ""
            )
            + "```"
        )
        embed.set_author(
            name=self.antinuke.cog_name or "No category",
        ...
        embed.add_field(
            name="Aliases",
            value=", ".join([f"`{alias}`" for alias in self.antinuke.aliases])
            or "`N/A`",
            inline=(False if len(self.antinuke.aliases) >= 4 else True),
        )
        embed.add_field(
            name="Parameters",
            value=", ".join([f"`{param}`" for param in self.antinuke.clean_params])
            or "`N/A`",
            inline=True,
        )
        embed.add_field(
            ...
            value=", ".join(
                [
                    ...
                    for check in self.antinuke.checks
                ]
            )
            or "`N/A`",
            inline=True,
        )
        if isinstance(self.antinuke, commands.Group):
            embed.add_field(
                name="Subcommands",
                value="\n".join(
                    [
    ...
    @antinuke.command(
        name="logs",
        description="configure the logs antinuke channel",
        help="antinuke",
    ...
    async def antinuke_logs(
        ...
        if not channel:
            ...
            return await ctx.send_success(
                "Antinuke logging channel set to server **owner dms**"
            )
        await self.bot.db.execute(
            "UPDATE antinuke_toggle SET logs = $1 WHERE guild_id = $2",
            channel.id,
        ...
        return await ctx.send_success(
            f"Antinuke logging channel set to {channel.mention}"
        )

    @antinuke.command(
        name="enable",
        aliases=["e", "on"],
        description="enable antinuke in your server",
        help="antinuke",
        brief="server owner",
    )
    @can_manage()
    ...
    @antinuke.command(
        name="disable",
        aliases=["d", "off"],
        description="disable antinuke in your server",
        help="antinuke",
        brief="server owner",
    )
    @can_manage()
    ...
    @antinuke_admin.command(
        ...
        brief="server owner",
        help="antinuke",
        description="add an antinuke admin",
        usage="[member]",
    )
    @can_manage()
    @is_antinuke()
    ...
    @antinuke_admin.command(
        name="list", help="antinuke", description="returns antinuke admins"
    )
    @is_antinuke()
    ...
    @antinuke.command(
        name="admins", help="antinuke", description="returns antinuke admins"
    )
    @is_antinuke()
    ...
    @antinuke.command(
        name="settings", aliases=["stats"], description="check antinuke settings"
    )
    @is_antinuke()
        ...
        embed = discord.Embed(
            ...
            description="\n".join(
                [
                    ...
                    for m in ["antibot", "ban", "channel", "kick"]
                ]
            ),
        )
    ...
    @antinuke.group(
        name="channel",
        invoke_without_command=True,
        description="prevent members from deleting the server's channels",
        help="antinuke",
    )
    ...
    @an_channels.command(
        name="enable",
        aliases=["e"],
        description="enable anti channel delete protection",
        usage="[punishment]",
        brief="antinuke admin",
        help="antinuke",
    )
    ...
    async def an_channels_enable(self, ctx: commands.Context, *, punishment: str):
        ...
        return await ctx.send_success(
            f"**Anti channel delete** has been enabled\npunishment: {punishment}\nThreshold: 1 channel deletion per 1 minute"
        )

    @an_channels.command(
        name="disable",
        aliases=["dis"],
        description="disable anti channel delete protection",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
    ...
    async def an_channels_disable(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Anti channel delete is **not** enabled")
        await self.bot.db.execute(
    ...
    @an_channels.command(
        ...
        usage="[number]",
        aliases=["limit", "count"],
        description="change the number of allowed channel deletions per 1 minute",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
    ...
    async def an_channels_threshold(self, ctx: commands.Context, number: int):
        ...
        return await ctx.send_success(
            f"Antinuke channel delete threshold changed to **{number}** channel deletes per **60** seconds"
        )

    @antinuke.group(
        name="kick",
        invoke_without_command=True,
        description="prevent members from mass kicking the server's members",
        help="antinuke",
    )
    ...
    @an_kick.command(
        name="enable",
        aliases=["e"],
        description="enable kick members protection",
        usage="[punishment]",
        brief="antinuke admin",
        help="antinuke",
    )
    ...
    async def an_kick_enable(self, ctx: commands.Context, *, punishment: str):
        ...
        return await ctx.send_success(
            f"**Anti kick**  has been enabled\npunishment: {punishment}\nThreshold: 1 kick per 1 minute"
        )

    @an_kick.command(
        name="disable",
        aliases=["dis"],
        description="disable kick protection",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
    ...
    async def an_kick_disable(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Antinuke kick is **not** enabled")
        await self.bot.db.execute(
            "DELETE FROM antinuke WHERE guild_id = $1 AND module = $2",
            ctx.guild.id,
            "kick",
    ...
    @an_kick.command(
        ...
        usage="[number]",
        aliases=["limit", "count"],
        description="change the number of allowed kicks per 1 minute",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
    ...
    async def an_kick_threshold(self, ctx: commands.Context, number: int):
        ...
        return await ctx.send_success(
            f"Antinuke kick threshold changed to **{number}** kicks per **60** seconds"
        )

    @antinuke.group(
        name="ban",
        invoke_without_command=True,
        description="prevent members from mass banning the server's members",
        help="antinuke",
    )
    ...
    @an_ban.command(
        name="enable",
        aliases=["e"],
        description="enable ban members protection",
        usage="[punishment]",
        brief="antinuke admin",
        help="antinuke",
    )
    ...
    async def an_ban_enable(self, ctx: commands.Context, *, punishment: str):
        ...
        return await ctx.send_success(
            f"**Antinuke Ban** has been enabled\npunishment: {punishment}\nThreshold: 1 ban per 1 minute"
        )

    @an_ban.command(
        name="disable",
        aliases=["dis"],
        description="disable ban protection",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
    ...
    async def an_ban_disable(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Antinuke ban is **not** enabled")
        await self.bot.db.execute(
            "DELETE FROM antinuke WHERE guild_id = $1 AND module = $2",
            ctx.guild.id,
            "ban",
    ...
    @an_ban.command(
        ...
        usage="[number]",
        aliases=["limit", "count"],
        description="change the number of allowed bans per one minute",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
    ...
    async def an_ban_threshold(self, ctx: commands.Context, number: int):
        ...
        return await ctx.send_success(
            f"Antinuke ban threshold changed to **{number}** bans per **60** seconds"
        )
    ...
    @botadd.command(
        ...
        brief="antinuke admin",
        help="antinuke",
        description="whitelist a bot so it can join the server ",
        usage="[member]",
    )
    @Perms.check_whitelist("antinuke")
    @is_antinuke()
    ...
    @botadd.command(
        ...
        help="antinuke",
        aliases=["uwl"],
        description="remove an antibot whitelisted member",
        usage="[member]",
    )
    @Perms.check_whitelist("antinuke")
    @is_antinuke()
    ...
    @botadd.command(
        name="whitelisted", help="antinuke", description="returns whitelisted bots"
    )
    @is_antinuke()
    ...
    @botadd.command(
        name="enable",
        aliases=["e"],
        description="enable bot adding protection",
        usage="[punishment]",
        brief="antinuke admin",
        help="antinuke",
    )
    ...
    async def botadd_enable(self, ctx: commands.Context, punishment: str):
        ...
        return await ctx.send_success(
            f"**Botadd** has been enabled\npunishment: {punishment}"
        )

    @botadd.command(
        name="disable",
        aliases=["dis"],
        description="disable antibot",
        help="antinuke",
        brief="antinuke admin",
    )
    @Perms.check_whitelist("antinuke")
