...
class AutoMod(commands.Cog):
                ...
                try:
                    ...
                    await message.channel.send(
                        embed=discord.Embed(
                            ...
                            title="vilan AutoMod",
                            description=f"{self.bot.warning} {message.author.mention}: You have been muted for **1 minute** for sending discord invites links",
                        )
                    )
                except KeyError:
                    pass
                ...
                try:
                    ...
                    await message.channel.send(
                        embed=discord.Embed(
                            color=self.bot.color,
                            title="vilan AutoMod",
                            description=f"{self.bot.warning} {message.author.mention}: You have been muted for **5 minutes** for sending discord invites links",
                        )
                    )
                except KeyError:
                    pass
    ...
    async def antiinvite_enable(self, ctx: commands.Context):
        ...
        if check:
            return await ctx.send_error("Anti-invite is **already** enabled")
        await self.bot.db.execute("INSERT INTO antiinvite VALUES ($1)", ctx.guild.id)
        return await ctx.send_success("Enabled anti-invite")

    @anti_invite.command(
        name="disable", description="disable anti invite", aliases=["d"], help="automod"
    )
    @Perms.get_perms("manage_guild")
    async def antiinvite_disable(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_error("Anti-invite is **not** enabled")
    ...
    @antiinvite_whitelisted.command(
        name="users", description="return a list of whitelisted users", help="automod"
    )
    ...
    async def anti_spam_enable(self, ctx: commands.Context):
        ...
        if check:
            return await ctx.send_error("Anti-spam is **already** enabled")
        await self.bot.db.execute(
    ...
    async def anti_spam_disable(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_error("Anti-spam is **not** enabled")
        await self.bot.db.execute(
    ...
    async def anti_spam_seconds(self, ctx: commands.Context, second: int):
        ...
        if second < 1:
            return await ctx.send_warning(
                f"Anti-spam delay can't be lower than 1 second"
            )
        await self.bot.db.execute(
    ...
    async def anti_spam_limit(self, ctx: commands.Context, second: int):
        ...
        if second < 1:
            return await ctx.send_warning(
                f"Anti-spam limit can't be lower than 1 second"
            )
        await self.bot.db.execute(
    ...
    @Perms.get_perms("manage_guild")
    async def as_unwhitelist_user(
        self, ctx: commands.Context, *, channel: discord.Member
    ):
        await Whitelist.unwhitelist_things(ctx, "antispam", channel)
    ...
    @chatfilter.command(
        name="list", description="returns a list of blacklisted words", help="automod"
    )
        ...
        for result in results:
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(
                        color=self.bot.color,
                        title=f"{ctx.guild.name} Blacklisted Words, ({len(results)})",
