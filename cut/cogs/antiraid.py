...
class AntiRaid(commands.Cog):
        ...
        embed = discord.Embed(
            ...
            description="\n".join(
                [
                    ...
                    for m in ["defaultavatar", "newaccounts", "massjoin"]
                ]
            ),
        )
    ...
    async def unwhitelist(self, ctx: commands.Context, *, member: discord.User):
        ...
        return await ctx.send_success(
            f"**{member}** can no longer use **anti raid** commands"
        )
    ...
    async def whitelisted(self, ctx: commands.Context):
        ...
        for result in results:
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(
                        color=self.bot.color,
                        title=f"{ctx.guild.name} Antiraid Whitelisted Users, ({len(results)})",
        ...
        number.append(
            discord.Embed(
                color=self.bot.color,
                title=f"a{ctx.guild.name} Antiraid Whitelisted Users, ({len(results)})",
                description=messages[i],
            )
        )
    ...
    @antiraid.group(
        ...
        help="antiraid",
        usage="[enable/disable] [punishment] [joins per 10 seconds]\nexample: antiraid massjoin enable 10",
    )
    ...
    async def massjoin_enable(self, ctx: commands.Context, punishment: str, joins: int):
        ...
        if check:
            return await ctx.send_warning("Massjoin is **already** enabled")
        await self.bot.db.execute(
        ...
        return await ctx.send_success(
            f"Massjoin is now enabled. This will be triggered only if there are more than **{joins}** joins under **10 seconds**\npunishment: **{punishment}**"
        )
    ...
    async def massjoin_disable(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Massjoin is **not** enabled")
        await self.bot.db.execute(
        ...
        try:
            if not punishment in ["kick", "ban"]:
                return await ctx.send_error(
                    f"Punishment should be either **kick** or **ban**, not **{punishment}**"
                )
