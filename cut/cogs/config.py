...
from tools.checks import Perms, Messages
from discord import (
    TextChannel,
    ChannelType,
    Embed,
...
from tools.utils import EmbedScript, Invoke
...
class Config(commands.Cog):
    ...
    @guildedit.command(
        name="name", description="edit the server's name", help="config", usage="[name]"
    )
    ...
    async def guildedit_description(self, ctx: commands.Context, *, desc: str = None):
        ...
        return await ctx.send_success(
            "Changed server's description to **{}**".format(desc)
        )

    @guildedit.command(
        name="icon",
        description="edit the server's icon",
        help="config",
        usage="[attachment]",
    )
    @Perms.get_perms("manage_guild")
    ...
    @guildedit.command(
        name="banner",
        description="edit the server's banner",
        help="config",
        usage="[attachment]",
    )
    @Perms.get_perms("manage_guild")
    ...
    @guildedit.command(
        name="splash",
        description="edit the server's splash",
        help="config",
        usage="[attachment]",
    )
    @Perms.get_perms("manage_guild")
    ...
    @commands.command(
        description="create an embed using the embed parser",
        help="config",
        usage="[code]",
        aliases=["ce"],
    )
    async def createembed(
        self, ctx: commands.Context, channel: discord.TextChannel, *, code: EmbedScript
    ):
        if channel == None:
            channel = ctx.channel
...
{useravatar} - returns user's profile picture
    """
        ...
        await ctx.paginator([embed1, embed2, embed3])
    ...
    @Perms.get_perms("manage_guild")
    async def autopfp_clear(self, ctx: commands.Context):
        check = await self.bot.db.fetchrow(
    ...
    async def autopfp_add(
        self, ctx: commands.Context, channel: TextChannel, genre: str, typ: str = "none"
    ):
        try:
            if genre in ["female", "male", "anime"]:
                ...
                return await ctx.send_error(
                    "The **genre** passed wasn't one of the following: male, female, anime, banner"
                )
        except:
            traceback.print_exc()
    ...
    async def autopfp_remove(
        self, ctx: commands.Context, genre: str, typ: str = "none"
    ):
        try:
            check = await self.bot.db.fetchrow(
                "SELECT * FROM autopfp WHERE guild_id = $1 AND genre = $2 AND type = $3",
        ...
        for result in results:
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(
                        color=self.bot.color,
                        title=f"{ctx.guild.name} Pingonjoin Channels, ({len(results)})",
    ...
    @starboard.command(
        help="config",
        description="set the starboard count",
        usage="[count]",
        aliases=["amount"],
    )
    @Perms.get_perms("manage_guild")
    async def count(self, ctx: commands.Context, count: int):
        if count < 1:
            return await ctx.send_warning("Count cannot be **less** than 1")
        check = await self.bot.db.fetchrow(
    ...
    async def starboard_emoji(
        ...
        emoji_id = (
            emoji.id if isinstance(emoji, discord.PartialEmoji) else ord(str(emoji))
        )
        if check is None:
            await self.bot.db.execute(
    ...
    @autorole.command(
        name="add",
        description="give a role to new users that joins your server",
        help="config",
        usage="[role]",
    )
    @Perms.get_perms("manage_guild")
    async def autorole_add(
        ...
        if isinstance(role, str):
            ...
            if role is None:
                return await ctx.send_error(
                    f"No role named **{ctx.message.clean_content[-len(ctx.clean_prefix)+15:]}** found"
                )
        check = await self.bot.db.fetchrow(
    ...
    async def autorole_remove(
        self, ctx: commands.Context, *, role: Union[discord.Role, str] = None
    ):
        if isinstance(role, str):
            role = ctx.find_role(role)
            if role is None:
                return await ctx.send_error(
                    f"No role named **{ctx.message.clean_content[-len(ctx.clean_prefix)+18:]}** found"
                )
        if role is not None:
            check = await self.bot.db.fetchrow(
                "SELECT * FROM autorole WHERE guild_id = {} AND role_id = {}".format(
    ...
    @confessions.command(
        name="remove", description="remove confessions channel", help="config"
    )
    @Perms.get_perms("manage_guild")
    async def confessions_remove(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning(
                "Confessions are **not** enabled in this server"
            )
        await self.bot.db.execute(
    ...
    @confessions.command(
        name="channel", description="check confessions channel", help="config"
    )
    ...
    @commands.command(
        description="set a server prefix", help="config", usage="[prefix]"
    )
    @Perms.get_perms("manage_guild")
    async def prefix(self, ctx: commands.Context, *, prefix: str):
        if len(prefix) > 3:
            return await ctx.send_error("Uh! The prefix is too long")
        check = await self.bot.db.fetchrow(
    ...
    @invoke.command(
        name="ban",
        help="config",
        description="add a custom ban message",
        usage="[message / embed]",
    )
    @Perms.get_perms("manage_guild")
    ...
    @invoke.command(
        name="kick",
        help="config",
        description="add a custom kick message",
        usage="[message / embed]",
    )
    @Perms.get_perms("manage_guild")
    ...
    @counter.command(
        name="types",
        description="check the counter types and channel types",
        help="config",
    )
    ...
    async def counter_remove(self, ctx: commands.Context, typ: str):
        if not typ in ["members", "humans", "bots"]:
            return await ctx.send_warning(f"**{typ}** is not an **available** counter")
        check = await self.bot.db.fetchrow(
        ...
        if not check:
            return await ctx.send_warning(
                f"There is no **{typ}** counter in this server"
            )
        ...
        for result in results:
            ...
            if l == 10:
                ...
                number.append(
                    discord.Embed(
                        color=self.bot.color,
                        title=f"{ctx.guild.name} Server Counters, ({len(results)})",
                        description=messages[i],
                    )
                )
                i += 1
    ...
    async def counter_add_members(
        self, ctx: commands.Context, typ: str, *, message: str = "{target}"
    ):
        if not typ in ["voice", "text"]:
            return await ctx.send_warning(f"**{typ}** is not a **valid** channel type")
        if not "{target}" in message:
            return await ctx.send_warning(
                "`{target}` is **missing** from the channel name"
            )
        check = await self.bot.db.fetchrow(
            "SELECT * FROM counters WHERE guild_id = $1 AND module = $2",
            ctx.guild.id,
    ...
    async def counter_add_humans(
        self, ctx: commands.Context, channeltype: str, *, message: str = "{target}"
    ):
        if not channeltype in ["voice", "text"]:
            return await ctx.send_warning(
    ...
    async def edit(self, ctx: commands.Context, *, role: Union[discord.Role, str]):
        ...
        options = [
            discord.SelectOption(label=perm.replace("_", " "), value=perm)
            for perm in perms
        ]
        embed = discord.Embed(
            color=self.bot.color,
            description="What perms should i add to {}?".format(role.mention),
        )
    ...
    async def permissions(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            name="avaible fakepermissions perms list",
        )
        await ctx.reply(embed=embed)
    ...
    @Perms.get_perms("manage_guild")
    async def mediaonly_add(self, ctx: commands.Context, *, channel: TextChannel):
        check = await self.bot.db.fetchrow(
    ...
    async def mediaonly_remove(
        self, ctx: commands.Context, *, channel: TextChannel = None
    ):
    ...
    async def mediaonly_list(self, ctx: commands.Context):
        i = 0
        k = 1
        l = 0
        mes = ""
        ...
        for result in results:
            ...
            if l == 10:
                ...
                number.append(
                    Embed(
                        color=self.bot.color,
                        title=f"{ctx.guild.name} Mediaonly Channels, ({len(results)})",
                        description=messages[i],
                    )
                )
                i += 1
