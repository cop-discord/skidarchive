        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if not check:
                return await interaction.client.ext.send_warning(
                    interaction,
                    "You do not have a **booster role** created. Use `boosterrole create` to create one",
                    ephemeral=True,
                )
            ...
            return await interaction.client.ext.send_success(
                interaction,
                "Changed your **booster role** name to **{}**".format(self.name.value),
                ephemeral=True,
            )
        ...
        async def on_submit(self, interaction: discord.Interaction):
            try:
                ...
                if not check:
                    return await interaction.client.ext.send_warning(
                        interaction,
                        "You don't have a booster role. Use `boosterrole create` to create one",
                        ephemeral=True,
                    )
                role = interaction.guild.get_role(check["role_id"])
                icon = ""
                ...
                return await interaction.client.ext.send_success(
                    interaction, "Changed your **booster role** icon", ephemeral=True
                )
            except:
    ...
    class Color(discord.ui.Modal, title="change the booster role color"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            try:
                ...
                return await interaction.client.ext.send_success(
                    interaction, "Changed your **booster role** color", ephemeral=True
                )
            except:
                return await interaction.client.ext.send_error(
    ...
    class Position(discord.ui.Modal, title="change your booster role position"):
        ...
        async def on_submit(self, interaction: discord.Interaction):
            ...
            if (
                ...
                return await interaction.client.ext.send_warning(
                    interaction, "I cannot managed this role", ephemeral=True
                )
            if int(self.name.value) == 0:
                return await interaction.client.ext.send_warning(
                    interaction, "Position cannot be lower than 1", ephemeral=True
                )
            await role.edit(position=int(self.name.value))
            return await interaction.client.ext.send_success(
                interaction,
                "Changed your **booster role** position to **{}**".format(
                    int(self.name.value)
                ),
                ephemeral=True,
            )
...
class Boosters(commands.Cog):
    ...
    @boosterrole.command(
        name="setup", help="config", description="set the boosterrole module"
    )
    ...
    async def br_setup(self, ctx: commands.Context):
        ...
        if check:
            return await ctx.send_warning(
                "Booster role module is **already** configured"
            )
        else:
            await self.bot.db.execute(
    ...
    async def br_remove(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="Are you sure you want to remove the boosterrole module? This action cannot be **undone**",
        )
        ...
        async def yes_callback(interaction: discord.Interaction):
            ...
            return await interaction.response.edit_message(
                embed=discord.Embed(
                    color=self.bot.color,
                    description=f"{self.bot.yes} {ctx.author.mention}: Booster role module has been cleared",
                ),
                view=None,
            )
    ...
    @boosterrole.command(
        description="set a base role for boosterrole module", help="config"
    )
    @Perms.get_perms("manage_guild")
    async def base(self, ctx: commands.Context, *, role: discord.Role = None):
        ...
        if not role:
            if not check:
                return await ctx.send_warning(
                    "Booster role module **base role** is not configured"
                )
            await self.bot.db.execute(
        ...
        if role.position >= ctx.guild.get_member(self.bot.user.id).top_role.position:
            return await ctx.send_warning("This role cannot be a booster role base")
        await self.bot.db.execute(
            "UPDATE booster_module SET base = $1 WHERE guild_id = $2",
            role.id,
            ctx.guild.id,
    ...
    @boosterrole.command(
        description="chnage the role position",
        help="config",
        usage="[position]",
        aliases=["pos"],
    )
    ...
    async def position(self, ctx: commands.Context, *, pos: int = 1):
        ...
        role = ctx.guild.get_role(check["role_id"])
        if role.position >= ctx.guild.get_member(self.bot.user.id).top_role.position:
            return await ctx.send_warning("This role cannot be managed by the bot")
        if pos == 0:
            return await ctx.send_warning("Position cannot be lower than 1")
        await role.edit(position=pos)
        return await ctx.send_success(
            "Changed your **booster role** position to **{}**".format(pos)
        )
    ...
    async def edit(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            description="customize your custom role with the options from the dropdown menu below",
        )
        ...
        async def select_callback(interaction: discord.Interaction):
            ...
            if select.values[0] == "cancel":
                ...
                return await interaction.client.ext.send_success(
                    interaction,
                    "Changed your **booster role** hoist status",
                    ephemeral=True,
                )
                ...
                await self.bot.ext.send_success(
                    interaction, "Your booster role has been deleted", ephemeral=True
                )
