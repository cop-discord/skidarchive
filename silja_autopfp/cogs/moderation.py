...
class mod(commands.Cog):
    ...
    if message.guild:
     async with self.bot.db.cursor() as cursor:
      ...
      if results is not None:
        ...
        if star is not None:
          await message.add_reaction("<:skull:1129811764779946065>")
        
   @commands.hybrid_command(help="role all users", usage="[subcommand] [target] [role]", description="moderation", brief="subcommands:\nremove - removes role from users\nadd - adds role to users\n\ntarget:\nhumans - targets human users\nbots - targets bot users\nall - targets all server users")
   ...
   async def roleall(self, ctx: commands.Context, subcommand=None, target=None, *, role: discord.Role=None):
    ...
    if subcommand == "remove":
      ...
      if target == "humans":
        ...
        message = await ctx.reply(embed=embed, mention_author=True, delete_after=50)
        try:
      ...
      if target == "humans":
        ...
        try:
         ...
         await message.edit(embed=discord.Embed(color=Colors.green, description=f"{Emojis.check} {ctx.author.mention}: added {role.mention} from all"))
          ...
          await message.edit(embed=discord.Embed(color=Colors.red, description=f"{Emojis.warning} {ctx.author.mention}: unable to add {role.mention} to all - {e}"))    
    else: return await commandhelp(self, ctx, ctx.command.name)
    
   @commands.hybrid_command(help="restore member's roles", usage="[member]", description="moderation")
   @commands.cooldown(1, 10, commands.BucketType.guild)
   @blacklist()
   ...
   @commands.hybrid_command(aliases=["yeet", "b"], help="ban a member", usage="[member] <reason>", description="moderation")
   @commands.cooldown(1, 10, commands.BucketType.guild)
   @blacklist()
      ...
      async with aiohttp.ClientSession(headers={"Authorization": f"Bot {self.bot.http.token}"}) as cs:
        async with cs.put(f"https://discord.com/api/v9/guilds/{ctx.guild.id}/bans/{member.id}") as r:
            if r.status == 204:
                ...
                em = discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention}: there was a problem banning this user {r.text}")    
                await sendmsg(self, ctx, None, em, None, None, None)

   @commands.hybrid_command(help="unban an user", description="moderation", usage="[member]")
   ...
   @commands.hybrid_command(aliases=["setnick", "nick"], help="change an user's nickname", usage="[member] <nickname>", description="moderation")
   ...
   async def nickname(self, ctx, member: discord.Member=None, *, nick=None):
    ...
    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
        e = discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention}: can't change nickname to members with higher roles than yours")
        await sendmsg(self, ctx, None, e, None, None, None)
        return
   ...
   @commands.hybrid_command(help="kick a member from the server", usage="[member] <reason>", description="moderation", aliases=["k"])
   @commands.cooldown(1, 10, commands.BucketType.guild)
   @blacklist()
    ...
    if ctx.guild.premium_subscriber_role in member.roles:
     ...
     async def button1_callback(interaction: discord.Interaction):
        ...
        try: 
         ...
         await interaction.response.edit_message(embed=no, mention_author=True)
     button1.callback = button1_callback
   ...
   @commands.hybrid_command(help="gives a role to a member", description="moderation", usage="[member] [role]")
   @commands.cooldown(1, 3, commands.BucketType.user) 
   @blacklist()
   ...
   @commands.hybrid_command(aliases=["sm"], help="add slowmode to a channel", description="moderation", usage="<channel>")
   @commands.cooldown(1, 3, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(help="lock a channel", description="moderation", usage="<channel>")
   ...
   @commands.hybrid_command(help="unlock a channel", description="moderation", usage="<channel>")
   @commands.cooldown(1, 4, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(aliases=["timeout", "m"], help="mute a member", usage="[member] [time] [reason]", description="moderation")
   @commands.cooldown(1, 4, commands.BucketType.user)
   @blacklist()
   async def mute(self, ctx: commands.Context, member: discord.Member=None, time=None, *, reason=None):
    ...
    try:
     ...
     await member.timeout(discord.classes.utcnow() + datetime.timedelta(seconds=time), reason=reason)
   ...
   @commands.hybrid_command(help="unmute a member", usage="[member]", description="moderation", aliases=["um"])
   @commands.cooldown(1, 5, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(help="removes all staff roles from a member", description="moderation", usage="[member]")
   @commands.cooldown(1, 5, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(help="bulk delete messages sent by bots", description="moderation", usage="[amount]", aliases=["bc", "botclear"])
   ...
   @commands.hybrid_command(help="bulk delete messages", description="moderation", usage="[amount] <member>")
   @commands.cooldown(1, 5, commands.BucketType.guild)  
   @blacklist()
   ...
   @commands.hybrid_command(help="auto delete member's messages", description="moderation", usage="[member]")
   ...
   @commands.hybrid_command(help="stops the auto delete messages of a member", description="moderation", usage="[member]")
   @commands.cooldown(1, 6, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(help="auto skull member's messages", description="moderation", usage="[member]", aliases=["skull"])
   @commands.cooldown(1, 6, commands.BucketType.user)
   @blacklist()
   async def skulls(self, ctx: commands.Context, member: discord.Member = None):
    if (not ctx.author.guild_permissions.moderate_members):
     await noperms(self, ctx, "timeout_members")
     return  
    if member == None:
     ...
     async with self.bot.db.cursor() as cursor:
      ...
      results = await cursor.fetchone()
      if results is not None:
       e = discord.Embed(color=Colors.default, description=f"{Emojis.warning} {ctx.author.mention} this member's messages are already getting skulled up <:skull:1129811764779946065>")
       await sendmsg(self, ctx, None, e, None, None, None)
       return
      elif results is None:
   ...
   @commands.hybrid_command(help="stops the auto skull messages of a member", description="moderation", usage="[member]", aliases=["unskull"])
   @commands.cooldown(1, 6, commands.BucketType.user)
   @blacklist()
   async def unskulls(self, ctx, member: discord.Member=None):
    if (not ctx.author.guild_permissions.moderate_members):
     await noperms(self, ctx, "timeout_members")
     return  
    if member == None:
        ...
        embed = discord.Embed(color=Colors.default, description=f"{Emojis.warning} {ctx.author.mention}: you can't unskull a member with higher roles than you")
        await sendmsg(self, ctx, None, embed, None, None, None)
        return        
    else:
     async with self.bot.db.cursor() as cursor:
