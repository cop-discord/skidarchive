   ...
   for a in activities:
    for l in list:
     #for t in emojiss:                                                                            #type=get_status(a)
      await bot.change_presence(status=discord.Status.online, activity=discord.CustomActivity(name=l, state=l))#, emoji=t)) ##activity=discord.Activity
...
async def female():
    ...
    eembed.add_field(name=" ", value=f"did you know? **{random.choice(responses)}**")
    ...
    async with bot.db.cursor() as cursor:
      ...
      for result in results:
        ...
        try:
                ...
                #wait asyncio.sleep(1.50)
                #wait msg.clear_reactions()
                ##wait asyncio.sleep(1.90)
                #wait msg.edit(embed=embed)
    ...
    async with bot.db.cursor() as cursor:
        ...
        for result in results:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(f"https://top.gg/api/bots/{bot.user.id}/check?userId={owner_id}", headers=auth) as r:
                    if r.status != 200:
...
async def stats():
    ...
    slay = bot.get_channel(1165643403917279345)
...
async def apistatus():
    ...
    embed.set_footer(text="use", icon_url="https://cdn.discordapp.com/attachments/1095072604668305481/1134204121176625232/i1aqqAG.gif")
    slay = bot.get_channel(1165643447227662336)
    await slay.send(embed=embed)
    msg = await slay.send(embed=embed1)
    #await msg.add_reaction("<a:w_w:1103337013656174652>")
    await asyncio.sleep(0.5)
...
class Client(commands.AutoShardedBot): #commands.AutoShardedBot 
  def __init__(self):
    super().__init__(
        ...
        owner_ids=[1094511683554267197, 749674886963920956, 733090221356220476, 461914901624127489]
    ) 
