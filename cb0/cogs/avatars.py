...
class avatars(commands.Cog):
    ...
    @commands.command(aliases=['avatars', 'oldavatars', 'avatarhistory', 'ah', 'avs'], description='utility', help='stalk users')
            ...
            async with self.bot.db.cursor() as cursor:
                ...
                if data:
                    for table in data:
                        ...
                        embeds.append(
                            discord.Embed(
                                title=f"{member.name}'s avatars",
                                color=0x2f3136
                            ).set_author(
                                name=f"{member.name}",
                                icon_url=member.avatar
                            ).set_footer(
                                text=f"{avatars}/{len(data)}",
                                icon_url=self.bot.user.avatar
                            ).set_image(url=username)
                        )
    ...
    @commands.command(aliases=['ca', 'clearavatar'], description='utility')
            ...
            async with self.bot.db.cursor() as cursor:
                ...
                if data:
                    ...
                    if before.display_avatar != after.display_avatar:
                        logs = self.bot.get_channel(1126568885605372064)
                        file = await before.display_avatar.to_file(
                            filename=f"{before.id}.{'png' if not before.display_avatar.is_animated() else 'gif'}"
                        )
