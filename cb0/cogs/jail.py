...
class jail(commands.Cog):
    ...
    @commands.hybrid_command(help="set the jail module", description="moderation")
    ...
    async def setme(self, ctx: commands.Context):
     ...
     async with self.bot.db.cursor() as cursor:
      ...
      role = await ctx.guild.create_role(name=f"{self.bot.user.name} - jail", color=0x000000)
    ...
    @commands.hybrid_command(description="moderation")
    @commands.cooldown(1, 6, commands.BucketType.guild)
    @blacklist()
    ...
    @commands.hybrid_command(help="jail a member", usage="[member]", description="moderation")
    ...
    async def jail(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
     ...
     if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention}: you can't jail {member.mention}")
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      chec = await cursor.fetchone()
      if chec is None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention} use `setme` command before using jail")
        await sendmsg(self, ctx, None, em, None, None, None) 
        return
      ...
      check = await cursor.fetchone()
      if check is not None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention}: {member.mention} is jailed already")
        await sendmsg(self, ctx, None, em, None, None, None) 
        return
    ...
    @commands.hybrid_command(help="unjail a member", usage="[member]", description="moderation")
    @commands.cooldown(1, 5, commands.BucketType.user)
    @blacklist()
    async def unjail(self, ctx, *, member: discord.Member=None):
     ...
     async with self.bot.db.cursor() as cursor: 
      ...
      check = await cursor.fetchone()
      if check is None:
        em = discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention}: {member.mention} is not jailed")
        await sendmsg(self, ctx, None, embed, None, None, None) 
        return
