...
class Events(commands.Cog):
    ...
    async def hii_events(self, guild: discord.Guild):
        ...
        button2 = discord.ui.Button(label="support", style=discord.ButtonStyle.url, url="https://discord.gg/usebot")
    ...
    async def leave_event_use(self, guild: discord.Guild):
      ...
      channel = self.bot.get_channel(1142039952297824368)
      msg = await channel.send(f"i left {guild.name} ({guild.id}) owned by {guild.owner} membercount is {guild.member_count} ppl and the database has been cleared *(event)")
      await msg.add_reaction("<:check:1126571720816468129>")
    ...
    async def on_message(self, message):
        ...
        if message.content == f"<@{self.bot.user.id}>":           
            ...
            for l in set(p for p in await self.bot.command_prefix(self.bot, message)):
             prefixes.append(l)
            await message.reply(content="hai :3 " + "\nprefix: <@1171112895250169937> and /slash")        
 ...
 #   @commands.Cog.listener('on_member_remove')
  #  async def booster_left(self, member: discord.Member): 
   #   if member.guild.id == 1094515015647772722:
    ...
    async def owos(self, event, error, *args, **kwargs):
        """This event is fired whenever an error occurs in the bot."""
        owo = self.bot.get_channel(1130527580924153966)
        msg = await owo.send(f"Error: {event}")

        # Add a reaction to the message to indicate that it is an error message
        await msg.add_reaction("<:error:1126572583442206820>")

    @commands.Cog.listener('on_command_error')
    async def eg(self, ctx, error): 
        owo = self.bot.get_channel(1130527580924153966)
        if isinstance(error,commands.errors.CommandNotFound): return
        elif isinstance(error, commands.errors.CheckFailure) : return
        else:
            try:
                msg = await owo.send(embed=discord.Embed(color=Colors.yellow, description=f"{Emojis.warning} {ctx.author.mention} {ctx.author.id} {ctx.gulid.id}: {error}"), mention_author=False)
            except: 
                pass
