...
#     @commands.hybrid_group(help="set an autoreact for the server", description="utility", usage="[subcommand] [trigger] [emoji]", brief="autoreact add - add an autoreact\nautoreact remove - remove an autoreact\nautoreact list - see a list of autoreact", aliases = ['art'])
#     async def autoreact(self, ctx):
#         if ctx.invoked_subcommand is None:
#             dev = self.bot.get_user(565627105552105507)
#             return await ctx.reply(f"{ctx.author.mention}: view the commands for **autoreact** on the main help embed")
