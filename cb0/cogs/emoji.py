...
class emoji(commands.Cog):
   ...
   @commands.hybrid_command(help="add an emoji", description="emoji", usage="[emoji] <name>", aliases=["ae"])
   ...
   @commands.hybrid_command(help="add multiple emojis", description="emoji", usage="[emojis]", aliases=["am"])
   ...
   async def addmultiple(self, ctx: commands.Context, *emoji: Union[discord.Emoji, discord.PartialEmoji]):
    if (not ctx.author.guild_permissions.manage_emojis_and_stickers):
     ...
     return
    if len(emoji) >= 50:
      embed=discord.Embed(color=Colors.default, description=f"{Emojis.warning} {ctx.author.mention} Unable to add more than 50 emojis at once (discord limitation)") 
   ...
   @commands.hybrid_command(help="add an emoji from an image link or attachment", description="emoji", usage="[image url] <name>", aliases=["imgemoji"])
   @commands.cooldown(1, 4, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(help="add a sticker", description="emoji", usage="[attach sticker / message link]", aliases=["addsticker"])
   @commands.cooldown(1, 4, commands.BucketType.user)
   @blacklist()
   ...
   @commands.hybrid_command(help="returns a list of server's emojis", description="emoji", aliases=["el"])
   ...
   @commands.hybrid_command(aliases=["downloademoji", "e"], help="gets an image version of your emoji", description="emoji", usage="[emoji]")
   @commands.cooldown(1, 4, commands.BucketType.user)
   @blacklist()
