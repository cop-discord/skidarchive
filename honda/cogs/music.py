...
class Music(commands.Cog):
    ...
    async def start_node(self):
      ...
      self.bot.node = await pomice.NodePool().create_node(bot=self.bot, host="69.46.10.250", port=2333, password="wetnakedboy123!", identifier="hondabot", secure=False, spotify_client_id="cbbe1d3cae1642dd8819e8746195c664", spotify_client_secret="6f09eb14821948bd8c5b185228bd1a0d", apple_music=True)
...
class Player(pomice.Player):
    ...
    async def next(self, no_vc: bool = False):
        ...
        if (channel := self.guild.get_channel(self.invoke_id)):
            await channel.send(embed=discord.Embed(color=0x2B2D31, description=f"<:check:1126571720816468129> {track.requester.mention}: Now Playing [{track.title}]({track.uri})"))
